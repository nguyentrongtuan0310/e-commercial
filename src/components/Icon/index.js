export const LocationIcon = ({ width = '22px', height = '22px', className }) => (
    <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 23.22 30.36"
        width={width}
        height={height}
        className={className}
    >
        <defs></defs>{' '}
        <g id="Layer_2" data-name="Layer 2">
            <g id="Layer_1-2" data-name="Layer 1">
                <path
                    d="M11.61.9A10.77,10.77,0,0,0,.9,11.69C.9,20.14,10.6,28.87,11,29.23a.87.87,0,0,0,1.18,0c.42-.36,10.12-9.09,10.12-17.54A10.77,10.77,0,0,0,11.61.9Z"
                    className="cls-1"
                ></path>{' '}
                <path d="M11.61,16.35a4.74,4.74,0,1,1,4.74-4.74,4.75,4.75,0,0,1-4.74,4.74Z" className="cls-1"></path>
            </g>
        </g>
    </svg>
);
export const CartIcon = ({ width = '25px', height = '25px', className }) => (
    <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 28.95 35.07"
        width={width}
        height={height}
        className={className}
    >
        {' '}
        <g id="Layer_2" data-name="Layer 2">
            <g id="Layer_1-2" data-name="Layer 1">
                <path
                    d="M10,10.54V5.35A4.44,4.44,0,0,1,14.47.9h0a4.45,4.45,0,0,1,4.45,4.45v5.19"
                    className="cls-1"
                ></path>{' '}
                <path
                    d="M23.47,34.17h-18A4.58,4.58,0,0,1,.91,29.24L2.5,8.78A1.44,1.44,0,0,1,3.94,7.46H25a1.43,1.43,0,0,1,1.43,1.32L28,29.24A4.57,4.57,0,0,1,23.47,34.17Z"
                    className="cls-1"
                ></path>
            </g>
        </g>
    </svg>
);
export const CategoryIcon = ({ width = '20px', height = '20px', className }) => (
    <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 26.99 26.99"
        height={height}
        width={width}
        className={className}
    >
        <defs></defs>{' '}
        <g id="Layer_2" data-name="Layer 2">
            <g id="Layer_1-2" data-name="Layer 1">
                <line x1="7.06" y1="7.52" x2="19.92" y2="7.52" className="cls-1"></line>{' '}
                <line x1="7.06" y1="13.49" x2="19.92" y2="13.49" className="cls-1"></line>{' '}
                <line x1="7.06" y1="19.47" x2="11.95" y2="19.47" className="cls-1"></line>{' '}
                <rect x="0.9" y="0.9" width="25.19" height="25.19" rx="4.71" className="cls-1"></rect>
            </g>
        </g>
    </svg>
);

export const CheckOrderIcon = ({ width = '35px', height = '35px', className }) => (
    <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 44.22 25.85"
        height={height}
        width={width}
        className={className}
    >
        <g id="Layer_2" data-name="Layer 2">
            <g id="Layer_1-2" data-name="Layer 1">
                <circle cx="7.66" cy="22.02" r="3" className="cls-1"></circle>{' '}
                <circle cx="24.79" cy="22.02" r="3" className="cls-1"></circle>{' '}
                <path d="M28.61,22h4.13a1.44,1.44,0,0,0,1.44-1.44v-6" className="cls-3"></path>{' '}
                <path
                    d="M27.12.9H14.82a1.44,1.44,0,0,0-1.44,1.44V20.58A1.44,1.44,0,0,0,14.82,22h6.29"
                    className="cls-3"
                ></path>{' '}
                <path
                    d="M10.27,5.92H6.75A2.65,2.65,0,0,0,4.54,7.11L1.33,12A2.64,2.64,0,0,0,.9,13.47v6.46A2.1,2.1,0,0,0,3,22h.75"
                    className="cls-3"
                ></path>{' '}
                <path d="M34.18,3.47V2.34A1.44,1.44,0,0,0,32.74.9H25.08" className="cls-3"></path>{' '}
                <line x1="43.32" y1="6.97" x2="32.57" y2="6.97" className="cls-3"></line>{' '}
                <line x1="40.5" y1="11.05" x2="32.57" y2="11.05" className="cls-3"></line>{' '}
                <line x1="42.09" y1="16.33" x2="38.07" y2="16.33" className="cls-3"></line>
            </g>
        </g>
    </svg>
);
export const CallIcon = ({ height = '24px', width = '24px', className }) => (
    <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 30.83 30.94"
        height={height}
        width={width}
        className={className}
    >
        <defs></defs>{' '}
        <g id="Layer_2" data-name="Layer 2">
            <g id="Layer_1-2" data-name="Layer 1">
                <path
                    d="M6.78,20.27a31,31,0,0,0,10.29,8.06A15.91,15.91,0,0,0,22.82,30h.41a4.77,4.77,0,0,0,3.7-1.59,0,0,0,0,0,0,0,14.62,14.62,0,0,1,1.17-1.2c.28-.28.57-.56.85-.85A2.91,2.91,0,0,0,29,22L25.33,18.4a2.94,2.94,0,0,0-2.13-1,3.07,3.07,0,0,0-2.15,1l-2.16,2.17c-.2-.12-.4-.22-.6-.32a6.74,6.74,0,0,1-.66-.36,22.82,22.82,0,0,1-5.47-5A13.11,13.11,0,0,1,10.32,12c.56-.52,1.09-1.05,1.61-1.58l.55-.56a3.07,3.07,0,0,0,1-2.17,3.08,3.08,0,0,0-1-2.18l-1.8-1.8L10.07,3c-.4-.41-.82-.83-1.23-1.21A3,3,0,0,0,6.72.9a3.07,3.07,0,0,0-2.15.94L2.31,4.09a4.64,4.64,0,0,0-1.38,3,11.09,11.09,0,0,0,.84,4.83,28.11,28.11,0,0,0,5,8.37Z"
                    className="cls-1"
                ></path>
            </g>
        </g>
    </svg>
);

export const SadIcon = () => (
    <svg
        aria-hidden="true"
        focusable="false"
        data-prefix="fas"
        data-icon="frown"
        role="img"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 496 512"
        className="svg-inline--fa fa-frown"
        data-v-108ccb56=""
    >
        <path
            fill="currentColor"
            d="M248 8C111 8 0 119 0 256s111 248 248 248 248-111 248-248S385 8 248 8zm80 168c17.7 0 32 14.3 32 32s-14.3 32-32 32-32-14.3-32-32 14.3-32 32-32zm-160 0c17.7 0 32 14.3 32 32s-14.3 32-32 32-32-14.3-32-32 14.3-32 32-32zm170.2 218.2C315.8 367.4 282.9 352 248 352s-67.8 15.4-90.2 42.2c-13.5 16.3-38.1-4.2-24.6-20.5C161.7 339.6 203.6 320 248 320s86.3 19.6 114.7 53.8c13.6 16.2-11 36.7-24.5 20.4z"
            data-v-108ccb56=""
            className=""
        ></path>
    </svg>
);
export const CartProcessIcon = ({ className }) => (
    <svg
        width="16"
        height="16"
        data-v-76247bda=""
        aria-hidden="true"
        focusable="false"
        data-prefix="fas"
        data-icon="shopping-cart"
        role="img"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 576 512"
        className="svg-inline--fa fa-shopping-cart"
    >
        <path
            data-v-76247bda=""
            fill="currentColor"
            d="M528.12 301.319l47.273-208C578.806 78.301 567.391 64 551.99 64H159.208l-9.166-44.81C147.758 8.021 137.93 0 126.529 0H24C10.745 0 0 10.745 0 24v16c0 13.255 10.745 24 24 24h69.883l70.248 343.435C147.325 417.1 136 435.222 136 456c0 30.928 25.072 56 56 56s56-25.072 56-56c0-15.674-6.447-29.835-16.824-40h209.647C430.447 426.165 424 440.326 424 456c0 30.928 25.072 56 56 56s56-25.072 56-56c0-22.172-12.888-41.332-31.579-50.405l5.517-24.276c3.413-15.018-8.002-29.319-23.403-29.319H218.117l-6.545-32h293.145c11.206 0 20.92-7.754 23.403-18.681z"
            className={className}
        ></path>
    </svg>
);
export const InfoOrderIcon = ({ className }) => (
    <svg
        width="16"
        height="16"
        data-v-76247bda=""
        aria-hidden="true"
        focusable="false"
        data-prefix="fas"
        data-icon="address-card"
        role="img"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 576 512"
        className="svg-inline--fa fa-address-card"
    >
        <path
            data-v-76247bda=""
            fill="currentColor"
            d="M528 32H48C21.5 32 0 53.5 0 80v352c0 26.5 21.5 48 48 48h480c26.5 0 48-21.5 48-48V80c0-26.5-21.5-48-48-48zm-352 96c35.3 0 64 28.7 64 64s-28.7 64-64 64-64-28.7-64-64 28.7-64 64-64zm112 236.8c0 10.6-10 19.2-22.4 19.2H86.4C74 384 64 375.4 64 364.8v-19.2c0-31.8 30.1-57.6 67.2-57.6h5c12.3 5.1 25.7 8 39.8 8s27.6-2.9 39.8-8h5c37.1 0 67.2 25.8 67.2 57.6v19.2zM512 312c0 4.4-3.6 8-8 8H360c-4.4 0-8-3.6-8-8v-16c0-4.4 3.6-8 8-8h144c4.4 0 8 3.6 8 8v16zm0-64c0 4.4-3.6 8-8 8H360c-4.4 0-8-3.6-8-8v-16c0-4.4 3.6-8 8-8h144c4.4 0 8 3.6 8 8v16zm0-64c0 4.4-3.6 8-8 8H360c-4.4 0-8-3.6-8-8v-16c0-4.4 3.6-8 8-8h144c4.4 0 8 3.6 8 8v16z"
            className={className}
        ></path>
    </svg>
);
export const VoucherIcon = ({ className }) => (
    <svg width="16" height="16" data-v-76247bda="" id="icon-voucher" viewBox="0 0 512 512" className={className}>
        <path
            data-v-76247bda=""
            d="M395.13,306.087c-9.206,0-16.696,7.49-16.696,16.696c0,9.206,7.49,16.696,16.696,16.696s16.696-7.49,16.696-16.696
      C411.826,313.577,404.336,306.087,395.13,306.087z"
        ></path>{' '}
        <path
            data-v-76247bda=""
            d="M261.565,172.522c-9.206,0-16.696,7.49-16.696,16.696s7.49,16.696,16.696,16.696c9.206,0,16.696-7.49,16.696-16.696
      S270.771,172.522,261.565,172.522z"
        ></path>{' '}
        <path
            data-v-76247bda=""
            d="M495.304,72.348H144.696v50.087c0,9.217-7.479,16.696-16.696,16.696s-16.696-7.479-16.696-16.696V72.348H16.696
      C7.479,72.348,0,79.826,0,89.044v333.913c0,9.217,7.479,16.696,16.696,16.696h94.609v-50.087c0-9.217,7.479-16.696,16.696-16.696
      s16.696,7.479,16.696,16.696v50.087h350.609c9.217,0,16.696-7.479,16.696-16.696V89.044C512,79.826,504.521,72.348,495.304,72.348
      z M144.696,322.783c0,9.217-7.479,16.696-16.696,16.696s-16.696-7.479-16.696-16.696v-33.391c0-9.217,7.479-16.696,16.696-16.696
      s16.696,7.479,16.696,16.696V322.783z M144.696,222.609c0,9.217-7.479,16.696-16.696,16.696s-16.696-7.479-16.696-16.696v-33.391
      c0-9.217,7.479-16.696,16.696-16.696s16.696,7.479,16.696,16.696V222.609z M211.478,189.217c0-27.619,22.468-50.087,50.087-50.087
      c27.619,0,50.087,22.468,50.087,50.087c0,27.619-22.468,50.087-50.087,50.087C233.946,239.304,211.478,216.836,211.478,189.217z
       M257.512,343.544c-4.271,0-8.544-1.631-11.804-4.892c-6.521-6.521-6.521-17.087,0-23.609L387.37,173.37
      c6.521-6.522,17.086-6.522,23.608,0c6.521,6.521,6.521,17.087,0,23.609L269.315,338.652
      C266.054,341.914,261.782,343.544,257.512,343.544z M395.13,372.87c-27.619,0-50.087-22.468-50.087-50.087
      c0-27.619,22.468-50.087,50.087-50.087s50.087,22.468,50.087,50.087C445.217,350.402,422.75,372.87,395.13,372.87z"
        ></path>
    </svg>
);
export const CreditCardIcon = ({ className }) => (
    <svg
        width="16"
        height="16"
        data-v-76247bda=""
        aria-hidden="true"
        focusable="false"
        data-prefix="fas"
        data-icon="credit-card"
        role="img"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 576 512"
    >
        <path
            data-v-76247bda=""
            fill="currentColor"
            d="M0 432c0 26.5 21.5 48 48 48h480c26.5 0 48-21.5 48-48V256H0v176zm192-68c0-6.6 5.4-12 12-12h136c6.6 0 12 5.4 12 12v40c0 6.6-5.4 12-12 12H204c-6.6 0-12-5.4-12-12v-40zm-128 0c0-6.6 5.4-12 12-12h72c6.6 0 12 5.4 12 12v40c0 6.6-5.4 12-12 12H76c-6.6 0-12-5.4-12-12v-40zM576 80v48H0V80c0-26.5 21.5-48 48-48h480c26.5 0 48 21.5 48 48z"
            className={className}
        ></path>
    </svg>
);
export const CompleteIcon = ({ className }) => (
    <svg
        width="16"
        height="16"
        data-v-76247bda=""
        aria-hidden="true"
        focusable="false"
        data-prefix="fas"
        data-icon="box-open"
        role="img"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 640 512"
        className="svg-inline--fa fa-box-open"
    >
        <path
            data-v-76247bda=""
            fill="currentColor"
            d="M425.7 256c-16.9 0-32.8-9-41.4-23.4L320 126l-64.2 106.6c-8.7 14.5-24.6 23.5-41.5 23.5-4.5 0-9-.6-13.3-1.9L64 215v178c0 14.7 10 27.5 24.2 31l216.2 54.1c10.2 2.5 20.9 2.5 31 0L551.8 424c14.2-3.6 24.2-16.4 24.2-31V215l-137 39.1c-4.3 1.3-8.8 1.9-13.3 1.9zm212.6-112.2L586.8 41c-3.1-6.2-9.8-9.8-16.7-8.9L320 64l91.7 152.1c3.8 6.3 11.4 9.3 18.5 7.3l197.9-56.5c9.9-2.9 14.7-13.9 10.2-23.1zM53.2 41L1.7 143.8c-4.6 9.2.3 20.2 10.1 23l197.9 56.5c7.1 2 14.7-1 18.5-7.3L320 64 69.8 32.1c-6.9-.8-13.5 2.7-16.6 8.9z"
            className={className}
        ></path>
    </svg>
);

export const HeartIconActive = ({ className }) => (
    <svg
        width="16"
        height="16"
        viewBox="0 0 19.2 18.5"
        aria-hidden="true"
        focusable="false"
        className={className}
        // className="heart-full icon-svg icon-svg--color-cps"
    >
        <path
            fill="currentColor"
            d="M9.66 18.48a4.23 4.23 0 0 1-2.89-1.22C.29 10.44-.12 7.79.02 5.67.21 2.87 1.95.03 5.42.01c1.61-.07 3.16.57 4.25 1.76A5.07 5.07 0 0 1 13.6 0c2.88 0 5.43 2.66 5.59 5.74.2 4.37-6.09 10.79-6.8 11.5-.71.77-1.7 1.21-2.74 1.23z"
        ></path>
    </svg>
);
export const HeartIcon = ({ className }) => (
    <svg width="16" height="16" viewBox="20 18 29 28" aria-hidden="true" focusable="false" className={className}>
        <path d="M28.3 21.1a4.3 4.3 0 0 1 4.1 2.6 2.5 2.5 0 0 0 2.3 1.7c1 0 1.7-.6 2.2-1.7a3.7 3.7 0 0 1 3.7-2.6c2.7 0 5.2 2.7 5.3 5.8.2 4-5.4 11.2-9.3 15a2.8 2.8 0 0 1-2 1 3.4 3.4 0 0 1-2.2-1c-9.6-10-9.4-13.2-9.3-15 0-1 .6-5.8 5.2-5.8m0-3c-5.3 0-7.9 4.3-8.2 8.5-.2 3.2.4 7.2 10.2 17.4a6.3 6.3 0 0 0 4.3 1.9 5.7 5.7 0 0 0 4.1-1.9c1.1-1 10.6-10.7 10.3-17.3-.2-4.6-4-8.6-8.4-8.6a7.6 7.6 0 0 0-6 2.7 8.1 8.1 0 0 0-6.2-2.7z"></path>
    </svg>
);

export const StarIcon = ({ className }) => (
    <svg height="15" width="15" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" className={className}>
        <path d="M381.2 150.3L524.9 171.5C536.8 173.2 546.8 181.6 550.6 193.1C554.4 204.7 551.3 217.3 542.7 225.9L438.5 328.1L463.1 474.7C465.1 486.7 460.2 498.9 450.2 506C440.3 513.1 427.2 514 416.5 508.3L288.1 439.8L159.8 508.3C149 514 135.9 513.1 126 506C116.1 498.9 111.1 486.7 113.2 474.7L137.8 328.1L33.58 225.9C24.97 217.3 21.91 204.7 25.69 193.1C29.46 181.6 39.43 173.2 51.42 171.5L195 150.3L259.4 17.97C264.7 6.954 275.9-.0391 288.1-.0391C300.4-.0391 311.6 6.954 316.9 17.97L381.2 150.3z"></path>
    </svg>
);
export const NewsIcon = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="15" viewBox="0 0 20 15" className={className}>
        <path
            id="newspaper"
            d="M17.5,6.5V4H0V17.75A1.25,1.25,0,0,0,1.25,19H18.125A1.875,1.875,0,0,0,20,17.125V6.5ZM16.25,17.75h-15V5.25h15ZM2.5,7.75H15V9H2.5Zm7.5,2.5h5V11.5H10Zm0,2.5h5V14H10Zm0,2.5h3.75V16.5H10Zm-7.5-5H8.75V16.5H2.5Z"
            transform="translate(0 -4)"
            fill="#d70018"
        ></path>
    </svg>
);
export const CheckIcon = ({ className }) => (
    <svg
        height="26"
        width="26"
        aria-hidden="true"
        focusable="false"
        data-prefix="fas"
        data-icon="check-circle"
        role="img"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 512 512"
        className={className}
    >
        <path
            fill="currentColor"
            d="M504 256c0 136.967-111.033 248-248 248S8 392.967 8 256 119.033 8 256 8s248 111.033 248 248zM227.314 387.314l184-184c6.248-6.248 6.248-16.379 0-22.627l-22.627-22.627c-6.248-6.249-16.379-6.249-22.628 0L216 308.118l-70.059-70.059c-6.248-6.248-16.379-6.248-22.628 0l-22.627 22.627c-6.248 6.248-6.248 16.379 0 22.627l104 104c6.249 6.249 16.379 6.249 22.628.001z"
        ></path>
    </svg>
);
export const UserIcon = ({ className }) => (
    <svg
        id="icon-smember"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 560 560"
        width="20"
        height="20"
        className={className}
    >
        <g id="Layer_2" data-name="Layer 2">
            <g id="Layer_1-2" data-name="Layer 1">
                <circle cx="280" cy="280" r="265" className="cls-1"></circle>{' '}
                <circle cx="280" cy="210" r="115" className="cls-1"></circle>{' '}
                <path
                    d="M86.82,461.4C124.71,354.71,241.91,298.93,348.6,336.82A205,205,0,0,1,473.18,461.4"
                    className="cls-1"
                ></path>
            </g>
        </g>
    </svg>
);
export const HomeIcon = ({ className }) => (
    <svg width="25" height="25" className={className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 17.71 17.72 ">
        {' '}
        <g id="Layer_2" data-name="Layer 2">
            <g id="Layer_1-2" data-name="Layer 1">
                <path
                    d="M.54,9.09V8.63a2.26,2.26,0,0,1,.75-1.16Q4.38,4.39,7.46,1.3A2.35,2.35,0,0,1,8.63.54h.46a2.28,2.28,0,0,1,1.17.76l6.35,6.35.2.21a1.54,1.54,0,0,1-.88,2.5,4.56,4.56,0,0,1-.54,0v4.91a1.79,1.79,0,0,1-1.86,1.87H11.05c-.41,0-.59-.19-.59-.6,0-1.25,0-2.49,0-3.74A.82.82,0,0,0,9.56,12c-.46,0-.92,0-1.38,0a.86.86,0,0,0-.93.92c0,1.24,0,2.48,0,3.72a.52.52,0,0,1-.59.59H4.22a1.8,1.8,0,0,1-1.9-1.9V10.41l0,0H2A1.49,1.49,0,0,1,.71,9.55,2.54,2.54,0,0,1,.54,9.09Z"
                    className="cls-1"
                ></path>
            </g>
        </g>
    </svg>
);
export const StoreIcon = ({ className }) => (
    <svg width="25" height="25" className={className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 17.71 17.72">
        <g id="Layer_2" data-name="Layer 2">
            <g id="Layer_1-2" data-name="Layer 1">
                <path
                    d="M4.22,17.18a1.8,1.8,0,0,1-1.9-1.9V10.41l0,0H2A1.49,1.49,0,0,1,.71,9.55a2.54,2.54,0,0,1-.17-.46V8.63a2.26,2.26,0,0,1,.75-1.16Q4.38,4.39,7.46,1.3A2.35,2.35,0,0,1,8.63.54h.46a2.28,2.28,0,0,1,1.17.76l6.35,6.35.2.21a1.54,1.54,0,0,1-.88,2.5,4.56,4.56,0,0,1-.54,0v4.91a1.79,1.79,0,0,1-1.86,1.87Z"
                    className="cls-1"
                ></path>
                <polygon
                    points="6.11 7.04 11.61 7.04 11.61 8.97 10.94 8.15 7.21 8.15 7.21 8.88 8.87 10.68 6.11 9.53 6.11 7.04"
                    className="cls-2"
                ></polygon>
                <polygon
                    points="6.11 12.24 6.11 14.26 11.61 14.26 11.61 11.36 8.89 10.08 10.5 11.92 10.5 13.15 6.86 13.15 6.11 12.24"
                    className="cls-2"
                ></polygon>
            </g>
        </g>
    </svg>
);
export const ReadMoreIcon = ({ className }) => (
    <svg width="25" height="25" className={className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16.47 6.77">
        <g id="Layer_2" data-name="Layer 2">
            <g id="Layer_1-2" data-name="Layer 1">
                <rect x="0.5" y="0.5" width="15.47" height="5.77" rx="2.89" className="cls-1"></rect>
                <circle cx="4.76" cy="3.39" r="0.18" className="cls-2"></circle>
                <circle cx="8.23" cy="3.39" r="0.18" className="cls-2"></circle>
                <circle cx="11.71" cy="3.39" r="0.18" className="cls-2"></circle>
            </g>
        </g>
    </svg>
);
export const BackIcon = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="26" height="30" className={className}>
        <path d="M447.1 256C447.1 273.7 433.7 288 416 288H109.3l105.4 105.4c12.5 12.5 12.5 32.75 0 45.25C208.4 444.9 200.2 448 192 448s-16.38-3.125-22.62-9.375l-160-160c-12.5-12.5-12.5-32.75 0-45.25l160-160c12.5-12.5 32.75-12.5 45.25 0s12.5 32.75 0 45.25L109.3 224H416C433.7 224 447.1 238.3 447.1 256z"></path>
    </svg>
);
